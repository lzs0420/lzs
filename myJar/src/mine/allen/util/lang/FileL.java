...
public class FileL {
	...
	public static byte[] toByteArray(String filePath){
		...
		LogL.getInstance().getLog().debug("filePath[BytesViewHandler]="+filePath);
		...
		try{
			...
			LogL.getInstance().getLog().debug("read file:" + filePath + "(size="+result.length+")");
		}
		catch(Exception e){
	...
	public static String getFullName(String FilePath, String sFileName){
		...
		if (sFullPath != null) {
			...
			dFile = new File(sFullPath);
			if (!dFile.exists()) {
				LogL.getInstance().getLog().error("at FileUtils.getFullName: 组合后文件[" + sFullPath + "]不存在！！！");
			}
		} else {
			LogL.getInstance().getLog().error("at FileUtils.getFullName: 文件 [" + FilePath + "]为空(null)！！！");
		}
 ...
 虑中。</b>
    ...
    public static void touch(File file) {
      ...
      if (!file.exists()) {
    	 LogL.getInstance().getLog().debug("file not found:" + file.getName()+".Create a new file.");
        try {
          if (file.createNewFile()) {
        	  LogL.getInstance().getLog().debug("Create a new file success!");
          }
          else {
        	  LogL.getInstance().getLog().error("Create file failed!");
          }
        }
        catch (IOException e) {
        	LogL.getInstance().getLog().error("Create file failed!", e);
        }
      }
      boolean result = file.setLastModified(currentTime);
      if (!result) {
    	  LogL.getInstance().getLog().error("touch failed: " + file.getName());
      }
    }
    /**
     * 修改文件的最后访问时间。
 ...
 虑中。</b>
    ...
    /**返回/WEB-INF(JAVAWEB项目)<br>
     * 或者/(JAVA项目)
     * @return ${classpath}的上一级目录(/bin/)(/WEB-INF/classes)
     */
    public static String getConfigAbsolutepath() {
    	String fileUrlPath = FileL.getPathPart(FileL.class.getResource("/").getPath());
